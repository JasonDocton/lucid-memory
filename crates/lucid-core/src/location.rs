...
pub fn compute_familiarity(access_count: u32, config: &LocationConfig) -> f64 {
    ...
    1.0 - 1.0 / (1.0 + config.familiarity_k * n)
}
...
pub fn compute_decayed_familiarity(
    ...
    let decay_rate = config.max_decay_rate * (1.0 - current_familiarity * config.decay_dampening);

    // Sliding floor (higher for well-known locations)
    let floor = if current_familiarity > 0.5 {
        config.base_floor + config.sticky_bonus * (current_familiarity - 0.5)
    } else {
        config.base_floor
    };
...
pub fn infer_activity_type(
    ...
    if let Some(tool) = tool_name {
        let tool_activity = match tool {
            "Read" => Some(ActivityType::Reading),
            "Edit" | "Write" => Some(ActivityType::Writing),
            "Grep" | "Glob" => Some(ActivityType::Reading),
            _ => None,
        };
...
pub fn compute_association_strength(
    ...
    1.0 - 1.0 / (1.0 + config.familiarity_k * effective_count)
}
