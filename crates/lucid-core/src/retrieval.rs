...
pub fn retrieve(input: &RetrievalInput<'_>, config: &RetrievalConfig) -> Vec<RetrievalCandidate> {
    ...
    let base_levels: Vec<f64> = input
        ...
        .enumerate()
        .map(|(i, history)| {
            let decay_rate = input.decay_rates.get(i).copied().unwrap_or(config.decay_rate);
    ...
    let mut candidates: Vec<RetrievalCandidate> = (0..n)
        .filter_map(|i| {
            ...
            let breakdown = combine_activations(
                base_level,
                probe_activation,
                spreading,
                emotional_weight,
...
#[must_use]
pub fn retrieve_by_similarity(probe_embedding: &[f64], memory_embeddings: &[Vec<f64>], top_k: usize) -> Vec<usize> {
